...
class Metric(object):
    ...
    def update(self, n, loss, outputs, targets):
        with torch.inference_mode():
            ...
            # values, indices = outputs.topk(k, dim = 1,  largest=True, sorted=True)
            # prediction = indices.T
            # targetse = targets.expand_as(prediction)
            # self._corrects_5 += prediction.eq(targetse) \
            #       .reshape(-1) \
            #       .float() \
    ...
    # @property
    # def accuracy_5(self):
    #     return (self._corrects_5 / self._n).item()
